...
bool rangeContains(
    ...
    const std::uintptr_t remaining = range.end - address;
...
bool matchPattern(
    ...
    if (!readableRangeContains(address, pattern.size())) {
        return false;
    }
    ...
    for (std::size_t i = 0; i < pattern.size(); ++i) {
        if (!pattern[i].wildcard && bytes[i] != pattern[i].value) {
            return false;
        }
    }
...
std::vector<std::uintptr_t> scanRange(
    ...
    if (!range.valid() || pattern.empty() || range.end - range.begin < pattern.size()) {
        return result;
    }

    const std::uintptr_t last = range.end - pattern.size();

    for (std::uintptr_t address = range.begin; address <= last; address += 4) {
...
std::vector<std::uintptr_t> scan(
    ...
    for (const auto& range : gScanRanges) {
        const auto matches = scanRange(range, pattern);
...
int scoreFunction(std::uintptr_t address) {
    if (!address)
        return 0;

    if (!readableRangeContains(address, sizeof(std::uint32_t) * 2)) {
        return 0;
    }
...
int phdrCallback(dl_phdr_info* info, std::size_t, void* user) {
    auto* wanted = static_cast<std::string*>(user);

    if (!info || !wanted || !info->dlpi_name)
        return 0;
    ...
    gModule.base = static_cast<std::uintptr_t>(info->dlpi_addr);
    ...
    for (std::size_t i = 0; i < info->dlpi_phnum; ++i) {
        ...
        const auto begin = gModule.base + static_cast<std::uintptr_t>(phdr.p_vaddr);
        const auto end = begin + static_cast<std::uintptr_t>(phdr.p_memsz);

        if (!gModule.textBegin || begin < gModule.textBegin) {
...
bool findReadableExecutableMappings(std::string_view libraryName) {
    gScanRanges.clear();
    ...
    while (std::fgets(line, sizeof(line), maps)) {
        ...
        const int parsed = std::sscanf(
            ...
            &beginRaw, &endRaw, permissions, &offsetRaw, path
        );
        ...
        while (!mappedPath.empty() && mappedPath.front() == ' ') {
            mappedPath.erase(mappedPath.begin());
        }

        constexpr std::string_view deletedSuffix = " (deleted)";
        if (mappedPath.size() >= deletedSuffix.size() &&
            mappedPath.compare(mappedPath.size() - deletedSuffix.size(), deletedSuffix.size(), deletedSuffix) == 0) {
            mappedPath.erase(mappedPath.size() - deletedSuffix.size());
        }

        const char* slash = std::strrchr(mappedPath.c_str(), '/');
        const char* filename = slash ? slash + 1 : mappedPath.c_str();

        if (std::string_view(filename) != libraryName) {
            continue;
        }

        const ScanRange range{
            static_cast<std::uintptr_t>(beginRaw),
            static_cast<std::uintptr_t>(endRaw)
        };
    ...
    std::sort(gScanRanges.begin(), gScanRanges.end(), [](const ScanRange& a, const ScanRange& b) {
        return a.begin < b.begin;
    });

    gScanRanges.erase(
        std::unique(gScanRanges.begin(), gScanRanges.end(), [](const ScanRange& a, const ScanRange& b) {
...
bool findModule(std::string_view libraryName) {
    std::string wanted(libraryName);
    gModule = {};
    gScanRanges.clear();

    dl_iterate_phdr(phdrCallback, &wanted);

    if (!gModule.valid()) {
        return false;
    }

    if (!findReadableExecutableMappings(libraryName)) {
        gModule = {};
        return false;
    }
...
std::uintptr_t resolveUnique(const Pattern& pattern, const char* name) {
    const auto matches = scan(pattern);
    if (matches.empty()) {
        LOGE("%s: no matches (pattern bytes=%zu)", name, pattern.size());
        return 0;
    }

    if (matches.size() > 1) {
        LOGI("%s: %zu matches; using first", name, matches.size());
    }

    const auto address = matches.front();
    LOGI("%s = %p", name, reinterpret_cast<void*>(address));
    return address;
}
...
bool initialize(std::string_view libraryName) {
    if (!findModule(libraryName)) {
        LOGE("Unable to locate %.*s", static_cast<int>(libraryName.size()), libraryName.data());
        return false;
    }

    LOGI("Minecraft base = %p", reinterpret_cast<void*>(gModule.base));

    gRenderLevel = resolveUnique(patternRenderLevel(), "RenderLevel");
    gClientInstanceUpdate = resolveUnique(patternClientInstanceUpdate(), "ClientInstanceUpdate");
    gClientInstanceGetLocalPlayer = resolveUnique(patternClientInstanceGetLocalPlayer(), "ClientInstanceGetLocalPlayer");
    gLevelGetHitResult = resolveUnique(patternLevelGetHitResult(), "LevelGetHitResult");
    gTessellatorBegin = resolveUnique(patternTessellatorBegin(), "TessellatorBegin");
    gTessellatorColor = resolveUnique(patternTessellatorColor(), "TessellatorColor");
    gTessellatorVertex = resolveUnique(patternTessellatorVertex(), "TessellatorVertex");
    gMeshRenderImmediately2 = resolveUnique(patternMeshRenderImmediately2(), "MeshHelpersRenderMeshImmediately2");
    gMeshRenderImmediately = resolveUnique(patternMeshRenderImmediately(), "MeshHelpersRenderMeshImmediately");

    gBlockCandidates.clear();
    const auto blockMatches = scan(patternBlockGetOutlineDiscovery());
    for (const auto address : blockMatches) {
        gBlockCandidates.push_back({address, scoreFunction(address)});
    }

    std::sort(gBlockCandidates.begin(), gBlockCandidates.end(), [](const Candidate& a, const Candidate& b) {
...
bool ready() {
    return gModule.valid() &&
           ...
           (gMeshRenderImmediately2 != 0 || gMeshRenderImmediately != 0);
}
...
std::uintptr_t resolve(Target target) {
    switch (target) {
        case Target::RenderLevel: return gRenderLevel;
        case Target::Tessellator: return gTessellatorBegin;
        default: return 0;
    }
}
...
const char* targetName(Target target) {
    switch (target) {
        case Target::RenderLevel: return "RenderLevel";
        case Target::Tessellator: return "Tessellator";
